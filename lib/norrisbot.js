...
const NorrisBot = function Constructor(settings) {
  ...
  this.dbPath = settings.dbPath || path.resolve(process.cwd(), 'data', 'norrisbot.js');
...
NorrisBot.prototype._firstRunCheck = function () {
  ...
  self.db.get('SELECT val FROM info WHERE name = "lastrun" LIMIT 1', function (err, record) {
    ...
    self.db.run('UPDATE into SET val = ? WHERE name = "lastrun"', currentTime);
  });
};

NorrisBot.prototype._welcomeMessage = function () {
  this.postMessageToChannel(this.channels[0].name, 'Hi guys, roundhouse-kink anyone?' +
...
NorrisBot.prototype._onMessage = function (message) {
  if (this._isChatMessage(message) &&
      this._isChannelConveration(message) &&
